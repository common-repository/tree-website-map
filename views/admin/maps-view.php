...
<?php $title = ( isset( $_GET['title'] ) && !empty( $_GET['title'] ) )? wm_sanitize_text_or_array_field($_GET['title']) : ''; ?>
<?php $type = ( isset( $_GET['type'] ))? wm_sanitize_text_or_array_field($_GET['type']) : NULL; ?>
<?php $limit = 7; ?>
		   ...
		   ?>
            <table class="table  table-striped table-hover wm-maps-table">
				...
				<?php foreach( $website_maps['data'] as $map ): ?>
					<tr style="height:25px" id="tr-mp_id-<?php echo (int) $map['mp_id'] ?>">
						
						<td style="width:40%"><img src="<?php echo plugins_url( 'images/' . strtolower( wm_sanitize_text_or_array_field($map['mp_type']) ) . '_16.png', WM_PLUGIN_MAIN_FILE ) ?>" border="0" width="16" height="16" />&nbsp; <a href="<?php echo $admin_url ?>admin.php?page=wm_edit_website_map&mpid=<?php echo (int) $map['mp_id'] ?>"><?php echo wm_sanitize_text_or_array_field($map['mp_name']); ?></a></td>
						<td>&nbsp;<?php echo wm_sanitize_text_or_array_field($map['mp_type']); ?></td>
						<td>&nbsp;[wm_website_map id="<?php echo (int) $map['mp_id'] ?>"]</td>
						<td style="width:5%">&nbsp;
							<a href="<?php echo $admin_url ?>admin.php?page=wm_edit_website_map&mpid=<?php echo (int) $map['mp_id'] ?>">
								Edit
							</a>
						</td>
						<td style="width:5%">
							<a href="javascript: void(0);" onclick="wm_DuplicateMap('<?php echo (int) $map['mp_id'] ?>');">Duplicate
							</a>
						</td>
