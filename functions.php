<?php
function wm_sanitize_text_or_array_field($array_or_string)
        {
...
function wm_duplicate_tree()
{
	...
	if($mp_id >0)
	{
	$wm_getMapsNextMaxId = wm_getMapsNextMaxId();
	
	
	$sql = "insert into `{$table_prefix}wm_maps` (SELECT ".$wm_getMapsNextMaxId.", concat(`mp_name`,' - Copy'), `mp_type`, `mp_options` FROM `{$table_prefix}wm_maps`  WHERE mp_id = ".$mp_id.")";
...
function wm_pagination($link, $total, $per_page, $current_page, $total_links = 5)
{
    ...
    for ($i = 1; $i <= $total_pages; $i++) {
        $out .= '<li ' . (($current_page == $i) ? ' class="active"' : '') . '>
			<a href="' . $link . $i . '" >&nbsp;' . $i . (($current_page == $i) ? '&nbsp;<span class="sr-only">&nbsp;</span>' : '') . '</a>
		</li>';
    }
...
function wm_create_new_horizontal_map()
{
    ...
    $name = 'horizontal map ' . wm_getMapsNextMaxId();
...
{
    global $wpdb;
    $ids = (isset($_POST['id'])) ? implode(', ', (int)$_POST['id']) : '';
    $ids = (empty($ids)) ? '0' : $ids;
...
{
    $action = (isset($_POST['action'])) ? wm_sanitize_text_or_array_field($_POST['action']) : '';
    $edit_ele_id = (isset($_POST['edit_ele_id'])) ? wm_sanitize_text_or_array_field($_POST['edit_ele_id']) : 0;
    $tr_parent_id = (isset($_POST['tr_parent_id'])) ? wm_sanitize_text_or_array_field($_POST['tr_parent_id']) : NULL;
    ...
    if ('wm_node_editform' == $action && $edit_ele_id > 0) {
        if ($node = wm_read_tree_node($edit_ele_id)) {
            ...
            $tr_title = wm_sanitize_text_or_array_field($node->tr_title);
            $tr_link = wm_sanitize_text_or_array_field($node->tr_link);
    ...
    <form class="<?php echo $formClass ?>" method="post" action="">
        ...
        <input type="hidden" name="tr_parent_id" value="<?php echo (int) $tr_parent_id ?>">
...
function wm_save_tree_node()
{
    ...
    $response = array();
    $data = array();
    $tr_id = (isset($_POST['id'])) ? (int)$_POST['id'] : 0;
    $data['tr_title'] = (isset($_POST['first_name'])) ? wm_sanitize_text_or_array_field($_POST['first_name']) : '';
    $data['tr_link'] = (isset($_POST['tr_link'])) ? wm_sanitize_text_or_array_field($_POST['tr_link']) : '';
    $tr_parent_id = (isset($_POST['tr_parent_id'])) ? wm_sanitize_text_or_array_field($_POST['tr_parent_id']) : NULL;
    ...
    $thumbnail_id = (isset($_POST['thumbnail_id'])) ?wm_sanitize_text_or_array_field($_POST['thumbnail_id']) : '';
	...
	$response = wm_sanitize_text_or_array_field($response);
...
function wm_change_parent_node()
{
    ...
    $tr_parent_id = (isset($_POST['tr_parent_id'])) ? wm_sanitize_text_or_array_field($_POST['tr_parent_id']) : 0;
...
function wm_edit_vertical_map()
{
    global $wpdb;
    $mp_id = (isset($_POST['mp_id'])) ? (int)$_POST['mp_id'] : 0;
    $mp_name = (isset($_POST['mp_name'])) ?wm_sanitize_text_or_array_field($_POST['mp_name']) : '';
    $mp_options = array();
    $mp_options['mp_enable_thumbnail'] = (isset($_POST['mp_enable_thumbnail'])) ?wm_sanitize_text_or_array_field($_POST['mp_enable_thumbnail']) : 0;
    $mp_options['mp_tab_link'] = (isset($_POST['mp_tab_link'])) ? (int)$_POST['mp_tab_link'] : 0;
    $mp_options['mp_enable_link'] = (isset($_POST['mp_enable_link'])) ? (int)$_POST['mp_enable_link'] : 0;
    $mp_options['mp_enable_totals'] = (isset($_POST['mp_enable_totals'])) ? (int)$_POST['mp_enable_totals'] : 0;
    $mp_options['mp_list_posts'] = (isset($_POST['mp_list_posts'])) ? (int)$_POST['mp_list_posts'] : 10;
    $mp_options['mp_container_width'] = (isset($_POST['mp_container_width'])) ?wm_sanitize_text_or_array_field($_POST['mp_container_width']) : 'auto';
    $mp_options['mp_container_height'] = (isset($_POST['mp_container_height'])) ?wm_sanitize_text_or_array_field($_POST['mp_container_height']) :
        'auto';
    $mp_options['state_data'] = (isset($_POST['stats'])) ?wm_sanitize_text_or_array_field($_POST['stats']) : '';
    $mp_options['mp_font_size'] = (isset($_POST['mp_font_size'])) ? (int)$_POST['mp_font_size'] : '22';
    $mp_options['mp_order_by'] = (isset($_POST['mp_order_by'])) ?wm_sanitize_text_or_array_field($_POST['mp_order_by']) : 'Date';
    $mp_options['mp_order_type'] = (isset($_POST['mp_order_type'])) ?wm_sanitize_text_or_array_field($_POST['mp_order_type']) : 'DESC';
    $default_options = get_option('wm_website_map_settings');
...
function wm_create_new_vertical_map()
{
    ...
    $name = 'vertical map ' . wm_getMapsNextMaxId();
...
{
	$thumb_url_ =wm_sanitize_text_or_array_field($_POST['thumb_url']);
    $thumb_url = (isset($_POST['thumb_url'])) ? urldecode($thumb_url_) : '';
...
function wm_get_vertical_tree_nodes()
{
    global $wpdb;
    $html = true;
    $mp_id = (isset($_GET['mp_id'])) ? (int)$_GET['mp_id'] : 0;
    $parent = (isset($_GET['parent'])) ? wm_sanitize_text_or_array_field($_GET['parent']) : NULL;
    $check_admin = (isset($_GET['check_admin'])) ? (int)$_GET['check_admin'] : NULL;
...
function wm_delete_vertical_tree_node()
{
    ...
    $data = array();
    $mp_id = (isset($_POST['mp_id'])) ? (int)$_POST['mp_id'] : 0;
    $nodes = (isset($_POST['nodes'])) ? wm_sanitize_text_or_array_field($_POST['nodes']) : array();
...
{
    ob_clean();
    $attach_id = (isset($_GET['atchid'])) ? (int)$_GET['atchid'] : NULL;
...
function wm_render_h_map_to_frame()
{
    ...
    $mp_id = (isset($_GET['mp_id'])) ? (int)$_GET['mp_id'] : NULL;
...
{
	wp_enqueue_style('bootstrap', WCMM_PLUGIN_URL . '/assets/css/style_css.css');
	wp_enqueue_script( 'script', plugins_url('js/js.js', WM_PLUGIN_MAIN_FILE), array(), 1.1, true);
}
...
{
    global $wpdb;
    $mp_id = (isset($_POST['mp_id'])) ? (int)$_POST['mp_id'] : NULL;
    $tr_id = (isset($_POST['tr_id'])) ? (int)$_POST['tr_id'] : NULL;
    $tr_parent_id = (isset($_POST['tr_parent_id'])) ? (int)$_POST['tr_parent_id'] : NULL;
    $tr_title = (isset($_POST['tr_title'])) ? wm_sanitize_text_or_array_field($_POST['tr_title']) : NULL;
    $tr_link = (isset($_POST['tr_link'])) ? wm_sanitize_text_or_array_field($_POST['tr_link']) : NULL;
    $tr_thumbnail_url = (isset($_POST['tr_thumbnail_url'])) ? wm_sanitize_text_or_array_field($_POST['tr_thumbnail_url']) : NULL;
...
function wm_move_ver_node()
{
    ...
    $tr_parent_id = (isset($_POST['tr_parent_id'])) ? (int)$_POST['tr_parent_id'] : NULL;
    $tr_order_old = (isset($_POST['tr_order_old'])) ?wm_sanitize_text_or_array_field($_POST['tr_order_old']) : NULL;
    $tr_order = (isset($_POST['tr_order'])) ?wm_sanitize_text_or_array_field($_POST['tr_order']) : NULL;

    $mp_id = (isset($_POST['mp_id'])) ? (int)$_POST['mp_id'] : 0;
    $records_update = (isset($_POST['record_update'])) ? wm_sanitize_text_or_array_field($_POST['record_update']) : array();
